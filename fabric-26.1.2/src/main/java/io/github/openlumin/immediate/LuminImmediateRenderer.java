...
/**
 * 即时模式渲染器 - OpenLumin 核心组件（26.1 基线）。
 *
 * 与 1.21.10 基线的差异：
 * - 采样器绑定改用 RenderPass.bindTexture(name, view, sampler)，采样器来自 LuminTexture/平台
 * - writeTransform 为 4 参数（无 lineWidth）
 * - 资源标识使用 Identifier
 *
...
public final class LuminImmediateRenderer {
    ...
    private static final Channel POS_COLOR_NORMAL_LINES = new Channel(DefaultVertexFormat.POSITION_COLOR_NORMAL_LINE_WIDTH, VertexFormat.Mode.LINES);
        ...
        private void drawAndReset() {
            ...
            try {
                ...
                try (RenderPass pass = RenderSystem.getDevice().createCommandEncoder().createRenderPass(
                    ...
                    // bindDefaultUniforms 绑定所有系统 UBO（Projection、Globals、Fog 等）
                    ...
                    if (this.texture != null) {
                        ...
                        if (textureObject != null) {
                            // 26.1.2：bindTexture 需显式携带采样器；
                            // 若为 LuminTexture（我们自己上传的）取其 view 与采样器，否则跳过
