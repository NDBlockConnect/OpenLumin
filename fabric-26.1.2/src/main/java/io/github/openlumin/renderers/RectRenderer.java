...
public class RectRenderer implements IRenderer {
        ...
        if (rectangleShader == null) {
            try {
                ...
                rectangleShader = ShaderProgram.load(
                    Identifier.fromNamespaceAndPath("openlumin", "shaders/rectangle.vsh"),
                    Identifier.fromNamespaceAndPath("openlumin", "shaders/rectangle.fsh"),
                    resourceManager
                );
            } catch (IOException e) {
    ...
    public void draw() {
        ...
        try (RenderPass pass = RenderSystem.getDevice().createCommandEncoder().createRenderPass(
                () -> "Rectangle Draw", info.colorView(), OptionalInt.empty(),
