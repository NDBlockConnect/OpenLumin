 ...
 * - GpuTexture/TextureFormat → textures 包
 * - 移除 GpuSampler（createSampler 已在1.21.10删除）
 * - LuminTexture 改用双参数构造（无 sampler）
 * - AddressMode/FilterMode 枚举来自 textures 包
 */
public class TtfGlyphAtlas {
    ...
    public TtfGlyphAtlas(int atlasId) {
        ...
        final var textureView = device.createTextureView(tex);

        // 1.21.10: createSampler 已移除，LuminTexture 使用双参数构造
        this.texture = new LuminTexture(tex, textureView);
        fillTextureWithTransparentDistance(tex);
     ...
     * Returns null if glyph atlas is full.
     */
    ...
    public LuminTexture getTexture() { return texture; }
