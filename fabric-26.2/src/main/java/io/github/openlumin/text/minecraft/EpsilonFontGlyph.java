...
/**
 * fabric-26.1.2 适配：
 * - BakedGlyph 为纯接口，createGlyph 直接返回 TextRenderable.Styled
 * - TextRenderable.render 签名为 Matrix4fc；VertexConsumer.addVertex 仅 float 重载，位姿需预变换
 * - 采样器为独立 GpuSampler，经 LuminTexture.getSampler() 提供
 * - 26.1.2 GlyphInfo 无 bake 方法
 */
public final class EpsilonFontGlyph implements BakedGlyph {
    ...
    // 26.1.2: BakedGlyph.createGlyph() 返回 TextRenderable.Styled
    @Override
    ...
    private final Vector3f vertexTmp = new Vector3f();
    ...
    private void renderGlyph(Matrix4fc pose, VertexConsumer buffer, GlyphInstance instance, float offsetX, float offsetY, float z, int color, boolean bold) {
        ...
        putVertex(pose, buffer, x0 + shearTop - extraThickness, y0 - extraThickness, z, uv.u0(), uv.v0(), color);
        putVertex(pose, buffer, x0 + shearBottom - extraThickness, y1 + extraThickness, z, uv.u0(), uv.v1(), color);
        putVertex(pose, buffer, x1 + shearBottom + extraThickness, y1 + extraThickness, z, uv.u1(), uv.v1(), color);
        putVertex(pose, buffer, x1 + shearTop + extraThickness, y0 - extraThickness, z, uv.u1(), uv.v0(), color);
    }

    private void putVertex(Matrix4fc pose, VertexConsumer buffer, float x, float y, float z, float u, float v, int color) {
        pose.transformPosition(x, y, z, vertexTmp);
        buffer.addVertex(vertexTmp.x, vertexTmp.y, vertexTmp.z).setUv(u, v).setColor(color);
    }
    ...
    private record GlyphInstance(
        ...
        // 26.1.2: GpuSampler 恢复为独立对象，经字形图集的 LuminTexture 提供
