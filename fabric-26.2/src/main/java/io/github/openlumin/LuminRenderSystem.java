...
/**
 * OpenLumin 渲染系统统一封装层（26.2 Vulkan 基底）。
 *
 * 与 26.1 基线的差异：
 * - 主渲染目标经 Minecraft.gameRenderer.mainRenderTarget() 获取
 * - 顺序索引缓冲按 PrimitiveTopology 获取
 * - 清屏颜色为 Vector4fc
 */
public class LuminRenderSystem {
    ...
    /**
     * 26.2：用 Projection + ProjectionMatrixBuffer 设置正交投影矩阵。
     */
    ...
    public static GpuBuffer getQuadIndexBuffer(int indexCount) {
        return PlatformRegistry.get().getSequentialBuffer(PrimitiveTopology.QUADS, indexCount);
    }

    public static com.mojang.blaze3d.IndexType getQuadIndexType() {
