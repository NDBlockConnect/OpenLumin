...
import io.github.openlumin.buffer.BufferUtils;
    ...
    private void emitGradientLayout(TextLayout layout, float x, float y, float scale, float width, int startArgb, int endArgb) {
        ...
        for (LayoutRun run : layout.runs) {
            ...
            for (int i = 0; i < run.glyphCount; i++) {
                ...
                int leftArgb = ARGB.toABGR(ARGB.lerp(clamp01(data[base + 8] / totalWidth), startArgb, endArgb));
                int rightArgb = ARGB.toABGR(ARGB.lerp(clamp01(data[base + 9] / totalWidth), startArgb, endArgb));
