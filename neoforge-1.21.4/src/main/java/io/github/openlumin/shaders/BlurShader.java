...
public class BlurShader {
    ...
    public void render(float x, float y, float width, float height, float rTL, float rTR, float rBR, float rBL, float blurStrength) {
        ...
        if (input == null) {
            input = new TextureTarget(targetWidth, targetHeight, false);
        }
    ...
    public void render3DBox(AABB box, double blurStrength) {
        ...
        if (input == null) {
            input = new TextureTarget(fb.width, fb.height, false);
        }
    ...
    private void addBoxVertices(BufferBuilder buffer, AABB box) {
        ...
        Matrix4f matrix = RenderSystem.getModelViewMatrix(); // Minecraft.getInstance().gameRenderer.getGameRenderState().levelRenderState.cameraRenderState.viewRotationMatrix;
