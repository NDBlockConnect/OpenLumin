...
public final class Render2DScheduler implements AutoCloseable {
    ...
    private void flushPreparedBatches(List<PreparedBatch> batches) {
        ...
        while (index < drawable.size()) {
            LuminPipeline pipeline = drawable.get(index).pipeline();
            int end = index + 1;
    ...
    private void flushPipelineRun(LuminPipeline pipeline, List<PreparedBatch> batches, int start, int end) {
        // NeoForge 1.21.4: 绕过 RenderSystemExtensions（运行时不存在），
        // 直接调用各 renderer 自己的 OpenGL draw() 路径。
        for (int i = start; i < end; i++) {
            batches.get(i).renderers().draw();
        }
    }

    private static LuminPipeline pipelineFor(Render2DCommandKind kind) {
        return switch (kind) {
    ...
    private record PreparedBatch(LuminPipeline pipeline, RendererBundle renderers) {
    }
